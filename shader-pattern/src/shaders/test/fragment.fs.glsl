...
void main() {
    ...
    // pattern 8
    // float strength = mod(vUv.y * 10.0, 1.0);
 // condition are bad for performance
    // if(strength < 0.5)
    //     strength = 0.0;
    // else
    //     strength = 1.0;
...
// pattern 25 
    // vec2 gridUv = vec2(floor(vUv.x * 10.0) / 10.0, floor((vUv.y+vUv.x-0.80) * 10.0) / 10.0);
    // float strength = random(gridUv);
// pattern 26
...
// pattern 30
    ...
    // float strength2 = 0.025/distance(lightUvY, vec2(0.5));
    // strength *= strength2;
    // Pattern 31
    // vec2 lightUvX = vec2(vUv.x *0.1 +0.45 , vUv.y *0.5 +0.25 );
    // float strength = 0.015 / distance(lightUvX, vec2(0.5));
